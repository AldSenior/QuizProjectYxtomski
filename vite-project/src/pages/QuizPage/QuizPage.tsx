...
export default function QuizPage({ onExit }: QuizPageProps) {
    ...
    const questions: Question[] = [
        {
            id: 1,
            question: "Как называется основной компонент в React Bootstrap для создания сетки?",
    ...
    if (isFinished) {
        return (
            <Container className="text-center py-5">
                <Card className="shadow-sm border-0 p-5" style={{ borderRadius: "24px" }}>
                    <h1 className="mb-4">Квиз завершен!</h1>
                    <h3 className="mb-4">
                        Ваш результат: <span className="text-success fw-bold">{score}</span> из{" "}
                        {totalQuestions}
                    </h3>
                    ...
                    <Col md={6}>
                        ...
                        <Form>
                            {currentQuestion.options.map((option, index) => (
                                <div
                                    key={index}
                                    className={`p-3 border rounded mb-2 d-flex align-items-center ${selectedOption === option
